              ...
              <div className="relative">
                <div className="w-full h-80 bg-brand-teal/20 rounded-3xl border border-white/10 backdrop-blur-sm overflow-hidden flex items-center justify-center">
                   <div className="text-white text-center p-8">
                     ...
                     <h3 className="text-2xl font-black">{t.NAAC_GRADE}</h3>
                     <p className="text-sm opacity-70">{t.HOSPITAL_ABOUT_BADGE_DESC}</p>
                   </div>
                </div>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-20">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-16">
          ...
          <div className="lg:col-span-2 space-y-12">
            <section>
              <h2 className="text-3xl font-black text-brand-blue mb-6 flex items-center">
            ...
            <section className="bg-brand-light p-10 rounded-3xl border border-blue-50">
              <h2 className="text-2xl font-black text-brand-blue mb-6">{t.HOSPITAL_SERVICE_PILLARS}</h2>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white border rounded-3xl shadow-xl overflow-hidden sticky top-6">
              <div className="bg-brand-blue p-6 text-white text-center">
                ...
                <p className="text-xs opacity-60">{t.LIVE_BULLETIN}</p>
              </div>
              <div className="p-6 space-y-6">
                {loading ? (
                  <div className="text-center py-10 opacity-40 italic">{t.SYNCING_CALENDAR}</div>
                ) : events.length > 0 ? (
                  events.map((evt, idx) => (
                    <div key={idx} className="flex gap-4 border-b pb-4 last:border-0">
                      <div className="bg-brand-light text-brand-blue p-2 rounded-xl text-center min-w-15 h-max">
                        <div className="text-lg font-black">{evt.date.split(' ')[0]}</div>
                        <div className="text-[10px] uppercase font-bold opacity-70">{evt.date.split(' ')[1]}</div>
                      </div>
                      <div>
                        <h4 className="font-bold text-xs text-brand-blue mb-1">{evt.title}</h4>
                        <span className="text-[10px] bg-brand-teal/10 text-brand-teal px-2 py-0.5 rounded-full font-black uppercase">{evt.type}</span>
                      </div>
                    </div>
                  ))
                ) : (
                  <div className="text-sm text-gray-400 text-center py-10">{t.NO_EVENTS_SCHEDULED}</div>
                )}
