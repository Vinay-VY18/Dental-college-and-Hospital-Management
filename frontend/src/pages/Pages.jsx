    ...
    <div className="flex flex-col min-h-screen bg-brand-light pt-12 pb-24">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        <div className="bg-white rounded-xl shadow-sm p-10 border border-gray-100 border-t-4 border-t-brand-teal">
          ...
          {loading ? (
            <div className="text-center text-gray-800 py-10">Syncing with Remote Database...</div>
          ) : data.length > 0 ? (
            <div className="space-y-6 mt-8">
              {data.map((item, idx) => (
                <div key={idx} className="bg-gray-50 p-6 rounded-lg border border-gray-100 shadow-sm">
                  ...
                  {item.facultyCount !== undefined && <p className="text-sm text-gray-800 font-semibold mb-2">Faculty Count: {item.facultyCount}</p>}
                  <p className="text-gray-700">{item.description}</p>
                </div>
              ))}
            </div>
          ) : (
            <div className="h-64 bg-gray-50 rounded-lg flex items-center justify-center text-gray-800 border border-dashed border-gray-300">
